(ns zoox.main
  "Kotodama WASM entrypoint for the Zoox clean-room actor (L4) — Clojure port.
  ...
  facts (`zoox.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"zoox.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "zoox-compat" :tier tier :entities entities} 200])
